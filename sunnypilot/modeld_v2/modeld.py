...
from openpilot.sunnypilot.modeld_v2.nv12_info import get_nv12_info
...
from openpilot.sunnypilot.modeld_v2.input_queues import derive_frame_skip, make_split_input_queues
...
class ModelState(ModelStateBase):
  ...
  def _init_combined(self, pkl_path, cam_w, cam_h, bundle):
    ...
    if 'model' in metadata:
      ...
      from openpilot.sunnypilot.modeld_v2.input_queues import make_supercombo_input_queues
...
def main(demo=False):
  ...
  while not vipc_client_main.connect(False) or not vipc_client_main.num_buffers:
    time.sleep(0.1)
  while use_extra_client and (not vipc_client_extra.connect(False) or not vipc_client_extra.num_buffers):
    time.sleep(0.1)
  ...
  while True:
    ...
    if sm.updated["liveCalibration"] and sm.seen['roadCameraState'] and sm.seen['deviceState']:
      ...
      intrinsics_main, intrinsics_extra = camera_offset_helper.get_intrinsics(sm, main_wide_camera)
