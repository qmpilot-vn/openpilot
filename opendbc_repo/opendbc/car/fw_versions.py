...
def get_fw_versions_ordered(can_recv: CanRecvCallable, can_send: CanSendCallable, set_obd_multiplexing: ObdCallback, vin: str,
  ...
  for brand in sorted(brand_matches, key=lambda b: (brand_matches[b].count(True), brand_matches[b].count(True) / len(brand_matches[b]) if len(brand_matches[b]) > 0 else 0), reverse=True):
    # Skip this brand if there are no matching present ECUs or no ECUs defined
    if len(brand_matches[brand]) == 0 or True not in brand_matches[brand]:
      continue
