...
def fingerprint(can_recv: CanRecvCallable, can_send: CanSendCallable, set_obd_multiplexing: ObdCallback, num_pandas: int,
  ...
  if not skip_fw_query:
    if cached_params is not None and cached_params.brand != "mock" and len(cached_params.carFw) > 0 and \
      ...
      cached = True
    elif fixed_fingerprint:
      # Platform already chosen (e.g. settings / fork default). OBD VIN & ECU FW queries are slow
      # and can block startup when multiplexing or buses are wrong; CAN fingerprint is enough.
      carlog.warning("Skipping VIN & FW query (fixed platform)")
