...
bool process_mads_heartbeat(SubMaster *sm) {
  if (!sm->allAliveAndValid({"selfdriveStateSP", "carParams"})) {
    return false;
  }
...
void can_send_thread(std::vector<Panda *> pandas, bool fake_send) {
  ...
  // Must match PubMaster queue_size for sendcan (see cereal/services.py); default 1MB here corrupts msgq vs 2MB publisher.
  std::unique_ptr<SubSocket> subscriber(SubSocket::create(context.get(), "sendcan", "127.0.0.1", false, true,
                                                          services.at("sendcan").queue_size));
  ...
  while (!do_exit && check_all_connected(pandas)) {
    ...
    // Match clock with Python can_list_to_can_capnp (time.monotonic -> CLOCK_MONOTONIC), not CLOCK_BOOTTIME.
    const uint64_t mono_nanos = nanos_monotonic();
    const int64_t age_ns = (int64_t)mono_nanos - (int64_t)event.getLogMonoTime();
    // Don't send if older than 1 second (signed avoids uint64 underflow if logMonoTime is slightly ahead)
    if (age_ns >= 0 && age_ns < (int64_t)1e9 && !fake_send) {
      for (const auto& panda : pandas) {
      ...
      LOGE("sendcan too old to send: mono_ns=%" PRIu64 ", logMonoTime=%" PRIu64 ", age_ns=%" PRId64,
           mono_nanos, event.getLogMonoTime(), age_ns);
    }
  }
}
...
void pandad_run(std::vector<Panda *> &pandas) {
  ...
  Params params;
  // Default 100 Hz; raise with PANDAD_MAIN_HZ (e.g. 200) if CAN RX is bursty (SPI/USB).
  const float main_hz = std::clamp(util::getenv("PANDAD_MAIN_HZ", 100.0f), 50.0f, 400.0f);
  const int peripheral_div = std::max(1, (int)std::lround(main_hz / 20.0));
  ...
  while (!do_exit && check_all_connected(pandas)) {
    ...
    // Process peripheral state at ~20 Hz (divisor scales with PANDAD_MAIN_HZ)
    if (rk.frame() % peripheral_div == 0) {
      process_peripheral_state(peripheral_panda, &pm, no_fan_control);
    }

    // Process panda state at ~10 Hz
    if (rk.frame() % panda_state_div == 0) {
      sm.update(0);
    ...
    if (rk.frame() % peripheral_state_div == 0) {
