...
def parse_model_output(model_output):
  ...
  for ds_suffix in ['lhd', 'rhd']:
    ...
    for key in ['face_prob', 'left_eye_prob', 'right_eye_prob', 'left_blink_prob',
                'right_blink_prob', 'sunglasses_prob', 'using_phone_prob', 'sleep_prob']:
...
def fill_driver_data(msg, model_output, ds_suffix):
  ...
  # This cereal keeps phoneProb / eyesClosedProb live; eye/ready fields are deprecated.
  msg.phoneProb = model_output[f'using_phone_prob_{ds_suffix}'][0, 0].item()
  msg.eyesClosedProb = model_output[f'sleep_prob_{ds_suffix}'][0, 0].item()
  left_eye = model_output[f'left_eye_prob_{ds_suffix}'][0, 0].item()
  right_eye = model_output[f'right_eye_prob_{ds_suffix}'][0, 0].item()
  msg.eyesVisibleProb = (left_eye + right_eye) * 0.5
...
def main():
  ...
  while VisionStreamType.VISION_STREAM_DRIVER not in VisionIpcClient.available_streams("camerad", block=False):
    time.sleep(0.1)

  vipc_client = VisionIpcClient("camerad", VisionStreamType.VISION_STREAM_DRIVER, True)
  while not vipc_client.connect(False) or not vipc_client.num_buffers:
    time.sleep(0.1)
