...
VISION_MATCH_THRESHOLD = 0.15   # min vision prob for radar+vision fusion (lower = fuse more)
VISION_ONLY_THRESHOLD  = 0.70   # min vision prob for vision-only lead (higher = radar-only fires more)
DIST_MATCH_FACTOR      = 0.55  # vision-radar distance tolerance factor (wider at range)
DIST_MATCH_MIN         = 8.0   # [m]  minimum distance tolerance
VEL_MATCH_REL          = 2.5   # [m/s] relative velocity tolerance
VEL_MATCH_ABS          = 18.0  # [m/s] absolute velocity tolerance (loose)
LAT_MATCH_FACTOR       = 3.5   # vision yStd multiplier for lateral tolerance
LAT_MATCH_MIN          = 3.0   # [m]  minimum lateral tolerance
RADAR_ONLY_MAX_DIST    = 120.0 # [m]  max distance for radar-only lead
RADAR_ONLY_MAX_LAT     = 2.6   # [m]  max |yRel| for radar-only lead (wider for bikes/motos)
STATIC_VEL_THRESHOLD   = 0.3   # [m/s] vision speed → static
STATIC_RADAR_VEL_MIN   = 5.0   # [m/s] radar speed above this triggers override
LEAD_HYSTERESIS_MARGIN = 0.10  # risk-score margin to switch leads (prevents jitter)
MAX_LEAD_DIST          = 140.0 # [m]  reject tracks beyond this
MAX_STATIC_LEAD_DIST   = 60.0  # [m]  reject static tracks beyond this
MAX_VREL_FILTER        = 30.0  # [m/s] reject high-speed tracks beyond 20 m
...
PATH_TOL_V          = [1.5, 1.9,  3.0,  4.0]      # [m] lateral tolerance at each BP
CLOSE_RANGE_CUTIN_TOL = 2.4  # [m] widened lane corridor for close, fast-approaching cut-ins
...
LOW_SPEED_MIN_TRACK_CNT = 3  # frames — admit new tracks sooner at low speed
CREEP_CLOSEST_LEAD_VEGO = 8.0 / 3.6  # [m/s] force-closest lead selection below 8 km/h
LOW_SPEED_PATH_MAX_LAT = 1.3         # [m] path gate below 8 km/h
CREEP_LEAD_MAX_DREL = 25.0           # [m] focus on nearby lead while creeping
CREEP_LEAD_MIN_TRACK_CNT = 2         # frames
CREEP_LEAD_MAX_YVREL = 1.4           # [m/s] reject strong lateral movers
CREEP_LEAD_MIN_VLEADK = -0.5         # [m/s] reject opposite-direction objects
...
LEAD_STABILITY_HOLD_FRAMES = 6
LEAD_STABILITY_MIN_DREL = 3.0   # [m] don't hold very-close obstacles
LEAD_STABILITY_BYPASS_TTC = 1.2 # [s] bypass hold only for truly imminent collision risk
LEAD_HOLD_MIN_VREL = -3.0       # [m/s] clamp very negative closing speed
LEAD_HOLD_MIN_ALEADK = -1.2     # [m/s²] clamp strong negative accel estimate
LEAD_HOLD_SIDE_YREL = 0.9       # [m] stricter clamp for laterally offset new leads
LEAD_HOLD_SIDE_MIN_VREL = -1.8  # [m/s] side-object clamp to avoid hard brake snap
LEAD_HOLD_SIDE_MIN_ALEADK = -0.5  # [m/s²] side-object accel clamp
...
NEAR_FIELD_STABLE_DREL = 3.5    # [m]
NEAR_FIELD_MIN_TRACK_CNT = 2    # frames
NEAR_FIELD_BYPASS_TTC = 1.1     # [s]
...
RADAR_LEAD_MIN_TRACK_CNT = 2
RADAR_LEAD_MIN_TRACK_CNT_LOW_SPEED = 3
RADAR_LEAD_BYPASS_TTC = 1.2
RADAR_LEAD_BYPASS_DREL = 12.0
...
SIDE_PASS_MAX_DREL = 35.0
SIDE_PASS_MIN_LAT = 0.6
SIDE_PASS_MIN_YVREL = 0.5
SIDE_PASS_MAX_TTC = 2.5
SIDE_PASS_MISS_LAT = 0.9

# Far-lateral (esp. right) approach: start slowing earlier but softer — avoid
# sudden hard brake when a crossing object is still offset.
# Radar yRel: positive = right of path.
FAR_LAT_SOFT_MIN_LAT = 0.70       # [m] path-relative |y| to treat as side approach
FAR_LAT_SOFT_MAX_LAT = 2.80       # [m]
FAR_LAT_SOFT_MIN_DREL = 10.0      # [m] keep soft response into mid range
FAR_LAT_SOFT_MAX_DREL = 65.0      # [m] begin soft response from farther out
FAR_LAT_SOFT_MIN_CLOSING = 0.8    # [m/s]
FAR_LAT_SOFT_VREL_FLOOR = -1.6    # [m/s] cap how "closing" the lead looks to MPC
FAR_LAT_SOFT_ALEADK_FLOOR = -0.40 # [m/s²] soft decel estimate
FAR_LAT_SOFT_DREL_PULL = 0.18     # pull dRel in so MPC reacts earlier but softly
FAR_LAT_RIGHT_BIAS = True         # prefer right-side (positive y) soft approach
...
def path_relative_yrel(track: "Track", path_y_offset: float = 0.0) -> float:
  """Track lateral offset from the planned driving path (openpilot: +right)."""
  return float(track.yRel - path_y_offset)


def is_far_lateral_soft_candidate(track: "Track", v_ego: float,
                                  path_y_offset: float = 0.0) -> bool:
  """True for far side approaches that should soft-brake, not slam or hard-reject.

  Radar ``yRel``: positive = right. Prefers passenger-side (right) urban crosses.
  Excludes clear opposite-corridor oncoming (those stay rejected).
  """
  y_rel_path = path_relative_yrel(track, path_y_offset)
  d_rel = float(track.dRel)
  closing = -float(track.vRel)

  if not (FAR_LAT_SOFT_MIN_DREL < d_rel < FAR_LAT_SOFT_MAX_DREL):
    return False
  if not (FAR_LAT_SOFT_MIN_LAT < abs(y_rel_path) < FAR_LAT_SOFT_MAX_LAT):
    return False
  if closing < FAR_LAT_SOFT_MIN_CLOSING:
    return False

  # Never soft-keep opposite-direction traffic as an ACC lead.
  if track.vLeadK < -1.0 or track.vRel < -(v_ego + 2.0):
    return False

  is_right = y_rel_path > FAR_LAT_SOFT_MIN_LAT
  if FAR_LAT_RIGHT_BIAS and not is_right:
    moving_toward = (y_rel_path * track.yvRel) < 0.0 and abs(track.yvRel) > 0.35
    if not moving_toward:
      return False
  return True


def soften_far_lateral_lead(lead_dict: dict[str, Any],
                            track: "Track",
                            v_ego: float,
                            path_y_offset: float = 0.0) -> dict[str, Any]:
  """Gentle early slowing for far-lateral (especially right) approaches.

  Goal: begin responding from farther away without a hard brake spike.
    * Pull ``dRel`` in slightly so MPC reacts earlier
    * Cap ``vRel`` / ``aLeadK`` so commanded decel stays soft
  Applies mainly to right-side (positive path-relative y) crossing-like tracks.
  """
  if (not lead_dict.get('status', False)) or (not lead_dict.get('radar', False)):
    return lead_dict
  if not is_far_lateral_soft_candidate(track, v_ego, path_y_offset):
    return lead_dict

  d_rel = float(lead_dict.get('dRel', track.dRel))
  v_rel = float(lead_dict.get('vRel', track.vRel))

  # Earlier reaction: present a slightly closer lead to MPC.
  d_soft = max(FAR_LAT_SOFT_MIN_DREL * 0.5, d_rel * (1.0 - FAR_LAT_SOFT_DREL_PULL))
  lead_dict['dRel'] = float(d_soft)

  # Softer kinematics: do not advertise a slam-brake closing rate.
  if v_rel < FAR_LAT_SOFT_VREL_FLOOR:
    v_rel = FAR_LAT_SOFT_VREL_FLOOR
    lead_dict['vRel'] = float(v_rel)
    lead_dict['vLead'] = float(v_ego + v_rel)
    lead_dict['vLeadK'] = float(v_ego + v_rel)

  a_lead = float(lead_dict.get('aLeadK', 0.0))
  if a_lead < FAR_LAT_SOFT_ALEADK_FLOOR:
    lead_dict['aLeadK'] = float(FAR_LAT_SOFT_ALEADK_FLOOR)
...
class Track:
  ...
  def potential_low_speed_lead(self, v_ego: float) -> bool:
    ...
    if self.vLeadK < 1.0 and v_ego > 1.5 and abs(self.yRel) > 1.2:
      return False
    if v_ego < LOW_SPEED_CUTOFF:
      return (abs(self.yRel) < 1.4 and
              abs(self.yvRel) < 0.8 and
              self.cnt >= LOW_SPEED_MIN_TRACK_CNT and
              (1.0 < self.dRel < 28))
    return (abs(self.yRel) < 1.9 and
            abs(self.yvRel) < 1.1 and
...
def is_crossing_traffic(track: Track, v_ego: float,
  ...
  # Far-right soft approach: keep as lead so we can slow gently from afar
  # instead of rejecting now and slamming later when it nears the path.
  if is_far_lateral_soft_candidate(track, v_ego, path_y_offset):
    return False
...
def get_lead(v_ego: float, ready: bool, tracks: dict[int, Track],
  ...
  # ── Priority 3: radar lead (path-gated) ─────────────────────────────────────
  # Prefer radar whenever an in-path track is available:
  #   * no lead yet, or
  #   * current lead is vision-only and radar is as close / slightly farther.
  # This keeps ACC following radar kinematics more often instead of vision-only.
  if tracks and ready and not lead_dict.get('radar', False):
    ...
    if best is not None and best.dRel < RADAR_ONLY_MAX_DIST:
      if is_track_in_planning_path(best, path_x, path_y, path_y_std, path_valid):
        vision_d = float(lead_dict.get('dRel', float('inf'))) if lead_dict.get('status', False) else float('inf')
        prefer_radar = (not lead_dict.get('status', False)) or (best.dRel <= vision_d + 8.0)
        # Still allow radar-only when vision is weak even if slightly farther.
        if (not prefer_radar) and lead_msg.prob <= VISION_ONLY_THRESHOLD:
          prefer_radar = True
        if prefer_radar:
          model_prob = float(lead_dict.get('modelProb', 0.0)) if lead_dict.get('status', False) else 0.0
          lead_dict = best.get_RadarState(model_prob, v_ego)
          lead_dict['fcw'] = False
  ...
  if lead_dict.get('status', False) and track_obj is not None:
    ...
    # Keep far-right soft approaches so soften_far_lateral_lead can slow gently.
    if (not is_far_lateral_soft_candidate(track_obj, v_ego, path_y_offset=pyo) and
            d_rel < 25.0 and lat_from_path > 1.2 and track_obj.vLeadK < 2.0 and
            v_rel > -1.5 and abs(track_obj.yvRel) > 0.35 and
  ...
  if lead_dict.get('status', False) and track_obj is not None:
    ...
    # Keep far-right soft approaches so soften_far_lateral_lead can slow gently.
    if (not is_far_lateral_soft_candidate(track_obj, v_ego, path_y_offset=pyo) and
            SIDE_PASS_MIN_DREL < d_rel < SIDE_PASS_MAX_DREL and
  ...
  if lead_dict.get('status', False) and lead_dict.get('radar', False):
    if track_obj is not None:
      # Path offset for far-lateral soft approach
      d_rel = float(lead_dict.get('dRel', 0.0))
      if path_valid and path_x is not None and path_y is not None:
        pyo, _ = get_path_lateral_offset(d_rel, path_x, path_y, path_y_std)
      else:
        pyo = 0.0
      lead_dict = soften_far_lateral_lead(lead_dict, track_obj, v_ego, path_y_offset=pyo)
      ...
      lead_dict = stabilize_position_only_radar_lead(lead_dict, track_obj, v_ego)
